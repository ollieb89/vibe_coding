...
> **Focus:** In-depth analysis of Claude Code slash commands, custom commands, subagent orchestration, and MCP integration  
> **Primary Sources:** Anthropic Official Docs, Skywork.ai, PubNub, Shipyard, GitHub community projects  
...
1. [[#Executive Summary]]
2. [[#Part 1: Built-in Slash Commands]]
3. [[#Part 2: Custom Commands (Markdown-based)]]
4. [[#Part 3: The Permission System]]
5. [[#Part 4: MCP Servers & Dynamic Commands]]
6. [[#Part 5: Subagent Orchestration & Delegation]]
7. [[#Part 6: Advanced Patterns & Workflows]]
8. [[#Part 7: CLAUDE.md Context Architecture]]
9. [[#Part 8: CLI Commands & Flags Reference]]
10. [[#Part 9: Best Practices & Optimization]]
11. [[#Appendix: Command Recipes & Examples]]
...
**The result:** You describe the goal, Claude Code orchestrates the right team.
...
**Use cases:** 
- Conversation has drifted too far from original task
- Need to reset mental model (switching projects mid-session)
...
**Pro tip:** `/clear` is drastic. Try `/compact` first to summarize instead of erase.
...
- Creates compressed summary of key decisions, code written, errors encountered
- Optionally preserves a focus area you specify
...
**Why it matters:**
- Claude Code automatically compacts when context approaches the token limit
- Manual compaction lets you control *what* gets preserved
- Prevents loss of nuance when automatic compaction kicks in
...
- Claude made an incorrect suggestion; rewind and provide correction
- Want to try a different approach from a known good point
...
**Strategy:**
- Main agent: `opus-4.1` (complex planning)
- Subagents: `haiku-4` (fast, parallelizable)
- Review phases: `sonnet-4.1` (balanced)
...
**Purpose:** Add additional working directories (useful for monorepos)  
**Mechanism:** Claude can now read/write from multiple directories in same session
...
→ Built-in Commands:
  ...
  /terminal-setup  Setup shell integration
  ...
  /review          Code review
  /plan            Break down task into steps
  /test:ui         Run UI tests
  /project:new     Scaffold new component
```
...
Custom commands are the **workflow layer** of Claude Code. They're Markdown files that become slash commands.

### Architecture Overview

**Command locations:**
```
~/.claude/commands/          → User-level (global, all projects)
  ├── commit.md              → /user:commit
  ├── review.md              → /user:review
  └── security/
      └── scan.md            → /user:security:scan
...
Every custom command is a Markdown file with YAML frontmatter.
...
model: opus                  # Can override global model
tools:                       # Optional: restrict tools for this command
  - Read
  - MCP(github)
---
...
|--------|---------|---------|
| `{cmd:shell_command}` | Execute shell, include output | `{cmd:git diff --staged}` |
| `{file:path}` | Include file content | `{file:src/App.tsx}` |
| `{{args}}` | User-provided arguments | `/fix-issue {{args}}` → args = issue ID |
| `{{env:VAR}}` | Environment variable expansion | `{{env:DATABASE_URL}}` |
| Markdown frontmatter | Configuration (name, model, tools) | `---` YAML `---` |

---

### Example 1: The Semantic Commit Command
...
### Example 2: The Task Planner Command
